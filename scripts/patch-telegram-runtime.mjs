...
function replaceOnce(source, from, to, label) {
  if (!source.includes(from)) throw new Error(`Telegram patch failed: pattern not found: ${label}`);
...
const newResolver = `  // Resolve Telegram targets exclusively from server-side environment configuration.\n  function resolveTelegramChatIds(_chatIdInput?: string | null): string[] {\n    const envChatId = (process.env.TELEGRAM_CHAT_ID || '').trim();\n    if (!envChatId) return [];\n    return Array.from(new Set(envChatId.split(/[,;\\n]+/).map((s) => s.trim()).filter((s) => /^-?\\d+$/.test(s))));\n  }\n\n  function getTelegramBotToken(): string {\n    const token = (process.env.TELEGRAM_BOT_TOKEN || '').trim();\n    if (!token) throw new Error('TELEGRAM_BOT_TOKEN is not configured on the server.');\n    return token;\n  }`;
if (server.includes('VERIFIED_FALLBACK_BOT_TOKEN')) server = replaceOnce(server, oldResolver, newResolver, 'server Telegram resolver/token block');
...
server = server.replace(`      const { order, chatId, botToken } = req.body;`, `      const { order } = req.body;`);
server = server.replace(`      const targetChatIds = resolveTelegramChatIds(chatId);`, `      const targetChatIds = resolveTelegramChatIds(undefined);`);
server = server.replace(`        inlineKeyboard,\n        botToken\n      );`, `        inlineKeyboard\n      );`);
server = server.replaceAll(`        res.json({\n          success: true,\n          telegramSent: false,`, `        res.status(502).json({\n          success: false,\n          telegramSent: false,`);
server = server.replace(`      const { chatId, botToken } = req.body;\n      const targetChatIds = resolveTelegramChatIds(chatId);`, `      const targetChatIds = resolveTelegramChatIds(undefined);`);
server = server.replace(`        testInlineKeyboard,\n        botToken\n      );`, `        testInlineKeyboard\n      );`);
server = server.replace(`console.log(\`• TELEGRAM_BOT_TOKEN env set: \${Boolean(process.env.TELEGRAM_BOT_TOKEN)} (\${maskToken(process.env.TELEGRAM_BOT_TOKEN || '')})\`);`, `console.log(\`• TELEGRAM_BOT_TOKEN configured: \${Boolean(process.env.TELEGRAM_BOT_TOKEN)}\`);`);
server = server.replace(`console.log(\`• TELEGRAM_CHAT_ID env set:   \${Boolean(process.env.TELEGRAM_CHAT_ID)} (\${process.env.TELEGRAM_CHAT_ID || 'not set, using default: 7460143967'})\`);`, `console.log(\`• TELEGRAM_CHAT_ID configured:   \${Boolean(process.env.TELEGRAM_CHAT_ID)}\`);`);
write('server.ts', server);

// CLIENT
let context = read('src/context/SiteDataContext.tsx');
context = replaceOnce(context,
`        body: JSON.stringify({\n          order,\n          botToken: telegramSettings.botToken,\n          chatId: telegramSettings.chatId,\n        }),`,
`        body: JSON.stringify({ order }),`, 'client order Telegram payload');
context = replaceOnce(context,
...
context = context.replace(`          ...data,\n          botToken: telegramSettings.botToken,\n          chatId: telegramSettings.chatId,\n        }),`, `          ...data,\n        }),`);
context = context.replace(`        success: result.success ?? true,`, `        success: Boolean(response.ok && result.success && result.telegramSent),`);
context = context.replace(`        success: true,\n        message: 'درخواست شما ثبت شد.',`, `        success: false,\n        message: 'درخواست ثبت شد اما ارسال اعلان تلگرام انجام نشد.',`);
// Do not persist a real Telegram destination/token in browser storage.
context = context.replace(`  chatId: '7460143967',`, `  chatId: '',`);
context = context.replace(`          botToken: safeToken,`, `          botToken: '',`);
write('src/context/SiteDataContext.tsx', context);

// The backend order endpoint already persists the order. Remove the extra browser-supplied Telegram fields from that sync call.
context = read('src/context/SiteDataContext.tsx');
context = context.replace(`        ...newOrder,\n        botToken: telegramSettings.botToken,\n        chatId: telegramSettings.chatId,`, `        ...newOrder,`);
write('src/context/SiteDataContext.tsx', context);

// UI: only show success after Telegram delivery is confirmed.
let modal = read('src/components/OrderModal.tsx');
modal = replaceOnce(modal,
